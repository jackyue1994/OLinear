if [ ! -d "./logs/orthoLinear/main_corr_only/Weather" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/Weather
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id Weather_OLinear_${seq_len}_${pred_len}_corr_only \
      --model $model_name \
      --data custom \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/Weather/Weather_OLinear_${seq_len}_${pred_len}_corr_only.log
