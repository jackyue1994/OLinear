if [ ! -d "./logs/orthoLinear/main_corr_only/ETTh1" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/ETTh1
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id ETTh1_OLinear_${seq_len}_${pred_len}_corr_only \
      --model $model_name \
      --data ETTh1 \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/ETTh1/ETTh1_OLinear_${seq_len}_${pred_len}_corr_only.log
done
