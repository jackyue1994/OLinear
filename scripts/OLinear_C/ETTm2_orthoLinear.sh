if [ ! -d "./logs/orthoLinear/main_corr_only/ETTm2" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/ETTm2
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id ETTm2_OLinear_${seq_len}_${pred_len}_corr_only \
      --model $model_name \
      --data ETTm2 \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/ETTm2/ETTm2_OLinear_${seq_len}_${pred_len}_corr_only.log
done
