if [ ! -d "./logs/orthoLinear/main_corr_only/robust/ECL" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/robust/ECL
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id ECL_OLinear_${seq_len}_${pred_len}_robust \
      --model $model_name \
      --data custom \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/robust/ECL/ECL_OLinear_${seq_len}_${pred_len}_robust.log
