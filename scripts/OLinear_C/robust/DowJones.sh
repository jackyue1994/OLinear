if [ ! -d "./logs/orthoLinear/main_corr_only/robust/DowJones" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/robust/DowJones
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id DowJones_OLinear_${seq_len}_${pred_len}_${d_model}_${lr}_robust \
      --model $model_name \
      --data custom \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/robust/DowJones/DowJones_OLinear_${seq_len}_${pred_len}_${d_model}_${lr}_robust.log
