if [ ! -d "./logs/orthoLinear/main_corr_only/robust/Wiki" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/robust/Wiki
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id Wiki_OLinear_${seq_len}_${pred_len}_robust \
      --model $model_name \
      --data custom \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/robust/Wiki/Wiki_OLinear_${seq_len}_${pred_len}_robust.log
