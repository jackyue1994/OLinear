if [ ! -d "./logs/orthoLinear/main_corr_only/robust/ETTm1" ]; then
    mkdir -p ./logs/orthoLinear/main_corr_only/robust/ETTm1
fi
...
do
    ...
    python -u run.py \
      ...
      --model_id ETTm1_OLinear_${seq_len}_${pred_len}_${learning_rate[i]}_robust \
      --model $model_name \
      --data ETTm1 \
      --features M \
      --seq_len ${seq_len} \
      ...
      2>&1 | tee -a logs/orthoLinear/main_corr_only/robust/ETTm1/orthoLinear_ETTm1_${seq_len}_${pred_len}_${learning_rate[i]}_robust.log
done
